    ...
    methods
        ...
        function [max_speed_table] = max_corner_speed(track_table)
            ...
            for track_inputs i=1:length(track_inputs)
                if abs(track_table(i,1)) = 1                                      %true if line in track table is a corner not a straight
                    syms v_max;                                             %variable to solve for
                    rho = track_table(i,2);                                    %corner radius for current corner
                    ...
                    acceleration_eq = (tire_coeff * (dfr + dfl + drr +drl)) / self.mass == lat_accel;     %available traction @ v_max == lateral accel @v_max: this is just an equation that is solved for the max velocity in the next line
        ...
        end        function [dfr, dfl, drr, drl] = weight_transfer_variable(self)
            %calculates the change in force on each tire due to
            %accelerations
            Cgz = self.veh_params.Cg_xyz(3);
            wheelbase = self.veh_params.wheelbase;
        ...
        function [motor_torque_used] = vd_main(max_motor_torque, time_step, straight_length)
            ...
            corner = track_table(k,1); %determine if in corner
            if corner == true
                dist_corner = 0;
                curr_velocity = max_speed_table(k,2);
            end
            ...
            if curr_velocity <= max_speed_table(k) && max_speed_table(k) >= low_velocity && corner == false %checks if car can still accelerate and be able to brake in time
                ...
                fric_force = coef * vehicle_mass/4; %%%%%needs more accurate normal force
                ...
                curr_velocity = (accel_force - RR_force - drag_force)/vehicle_mass * time_step; %accelerate car

            elseif curr_velocity > max_speed_table(k) && corner == false %checks if car needs to brake
                curr_velocity = curr_velocity - (19.62*time_step); %brakes car at -2G
